...
namespace BSEBExamResult_QRGenerate.Controllers
{
    ...
    public class ProvisionalEXAMQREncData : ControllerBase
    {
        ...
        // ✅ Bulk encrypt ALL students and save to DB
        [HttpPost("CompartGenerateProvisionalEXAMQREncData")]
        public async Task<IActionResult> CompartGenerateProvisionalEXAMQREncData()
        {
            _logger.LogInformation("Bulk QR Encryption started at {Time}", DateTime.Now);
            ...
            foreach (var (rollCode, rollNo) in allRolls)
            {
                try
                {
                    ...
                    string encrypted = string.Equals(student.ExamType, "COMPARTMENTAL", StringComparison.OrdinalIgnoreCase)
                       ? "P2C" + QrUtility.CompartGenerateProvisionalEncryptedPayload(student)
                       : "P2S" + QrUtility.CompartGenerateProvisionalEncryptedPayload(student);
