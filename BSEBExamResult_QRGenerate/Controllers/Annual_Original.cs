...
namespace BSEBExamResult_QRGenerate.Controllers
{
    ...
    [ApiController]
    public class Annual_Original : ControllerBase
    {
        #region Jinal Nagar
        ...
        public async Task<IActionResult> GenerateAnnualOriginalEXAMQREncData()
        {
            ...
            var batch = new List<AnnualOriginalEXAMQREncdData>(BATCH_SIZE);
            ...
            foreach (var (rollCode, rollNo) in allRolls)
            {
                try
                {
                    ...
                    var student = await _dbHelper.GetAnnualStudentResultAsync(rollCode, rollNo);
                    ...
                    string encrypted = "5O" + QrUtility.GenerateAnnualOriginalEncryptedPayload(student);
                    ...
                    //});
                    batch.Add(new AnnualOriginalEXAMQREncdData
                    {
                        RollCode = rollCode,
                        RollNo = rollNo,
                        EncryptedData = encrypted,
        ...
        [HttpPost("CompartGenerateAnnualOriginalEXAMQREncData")]
        public async Task<IActionResult> CompartGenerateAnnualOriginalEXAMQREncData()
        {
            _logger.LogInformation("Bulk QR Encryption started at {Time}", DateTime.Now);
